function get_Adminer_mysql_info()
{
	$.ajax({
			...
			async: false,
			cache: false,		
			data: {cmd:"module_Get_One_Info",f_module_name:'Adminer-mysql'},	
			dataType:"xml",
...
function check_button() 
{
    $.ajax({
			...
			{	
					...
					if (my_state == 0)
						$("#But_WEB_IF").removeClass("button_medium").addClass("button_medium_display");
				    else
				        $("#But_WEB_IF").removeClass("button_medium_display").addClass("button_medium");
			}
	});
}
...
//	    my_url = "http://"+document.domain+":4711?pass=admin";
	...
	if(my_url.indexOf(":")==-1)
	{
	    my_url = "http://"+document.domain+"/Adminer-mysql/adminer.php?username=root";
    	window.open(my_url,"Adminer-mysql");
    }
    else
    {
