...
$(window).bind("hashchange", function(e){
    ...
    switch (e.fragment) {
        ...
        case "multiusersupport":
            checkChanges("loadMultiUserSupport();");
            populateSettingsNav();
            break;
        case "advanced":
...
function loadStatus(isInitial){
	...
	makeGetRequest("/rpc/get_friendlyname", {}, function(response, parameter){
...
function buildViewNodesFor(nodetype) {
	var html = "";
	var namenodes = "viewNodeName";
	var nodes = getViewNodes(nodetype);
    ...
    makeGetRequest("/rpc/info_clients", {}, function(response, parameter){
        ...
        if (expectedCalls == returnedCalls) {
			// get the multi user support data and call the function loadSharingHtml
 			getMultiUserSupport_Data(sharing["multiusersupportenabled"], sharing["multiuserusehttps"], loadSharingHtml);
        }
    });
...
function loadMultiUserSupport(){
    ...
    makeGetRequest("/rpc/get_all", {}, function(response, parameter){
        ...
        if (expectedCalls == returnedCalls) {
			...
			getMultiUserSupport_Data(multiUserSupport["multiusersupportenabled"], multiUserSupport["multiuserusehttps"], loadMultiUserSupportHtml);
        }
    });
	// get the tls port for https
    makeGetRequest("/rpc/get_tls_port", {}, function(response, parameter){
...
function getMultiUserSupport_Data(multiUserSupportEnabled, multiUserUseHttps, callFunction) {
	...
	switchToHttps(multiUserUseHttps);	// switch from http to https
	// multi user support is enabled. Read the user, role, location and device data.
    returnedCalls = 0;
    expectedCalls = 3;
	showLoadingGraphic();
...
function loadMultiUserSupportHtml(){
    ...
    function(parameter) {
		...
		showDialogOverlay(function(){
			...
			text: getString("ok"),
			onclick: "hideDialogOverlay();switchToHttp('#status', " + multiUserSupport["multiuserusehttps"] + ");"
		}]);		       
    });
}

function switchToHttps(multiUserUseHttps) {
	if (multiUserUseHttps == 0) return;		// https disabled, do not switch between http and https
	var href = window.location.href;
	if (window.location.protocol == "https:") return;
	var port = multiUserSupport["tlsport"];
	var newUrl = 'https:\/\/'+window.location.hostname+':'+port+window.location.pathname+window.location.hash;
	window.location.assign(newUrl);
}
function switchToHttp(hash, multiUserUseHttps) {
	if (multiUserUseHttps == 0) {
		// https disabled, do not switch between http and https
		navigateTo(hash);
		return;
	}
	var href = window.location.href;
	...
	var port = '9000';
	var newUrl = 'http:\/\/'+window.location.hostname+':'+port+window.location.pathname+hash;
	window.location.assign(newUrl);
	navigateTo(hash);
}
...
function finishSavingUserAndRoles2() {
	if (saveFlag) {
		...
		makePostRequest2("/rpc/set_all", {}, data, function(response, parameter){
			...
			hideLoadingGraphic();
			finishSaving();
			switchToHttp("#multiusersupport", multiUserSupport["multiuserusehttps"]);
		});		
	} else {
		requestFailed();
		hideLoadingGraphic();
