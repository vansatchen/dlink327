...
function playOnLocalDevice(bookmark, url, elemUrl, title, artist, poster) {
	// music
	if (currentPersistentID == persistentIDMusic) {
		if (!document.getElementById("jPlayerPlayList")) {
			if (!initJPlayerBoxForLocalPlay()) return;		// init jPlayer
		}
		
		showLocalDeviceBox(true);
	...
	// video
	if (currentPersistentID == persistentIDVideo) {
...
function playContainerOnLocalDevice(url) {
...
function buildLocalDeviceHeader() {
	var closeButton = '<a class="jp-closebutton" title="' + getString("closelocaldevicebox") + '" onclick="closeLocalDeviceBox()"></a></div>';
	var html;
...
function showLocalDeviceBox(showAlways) {
