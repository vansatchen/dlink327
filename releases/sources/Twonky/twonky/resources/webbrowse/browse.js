...
function logout() {
	...
	selectServer(serverURL, escape(serverTitle), serverBookmark, multiusersupportenabled);
}
...
function buildScreen(hash) {
	if (hash == "") {
		...
		if (url0 == "") {
			showNavigationError();
		} else loadMediaBrowseContent(url0);			// get the content and display it
	} else {
...
function populateServersList() {
    ...
    for (var i = 0; i < itemCount; i++) {
		...
		html += '<li name="leftServerContainer" id="' + id + '" serverurl="' + url + '"onclick="selectServer(\'' + url + '\', \'' + escape(title) + '\', \'' + bookmark + '\', \'' + musEnabled + '\')"><a>' + title + '</a></li>';
...
function selectServer(url, titleEscaped, bookmark, musEnabled) {
    ...
    try {
        ...
        for (i = 0; i < elem.children.length; i = i + 2) {
            removeClass(elem.children[i].id, "current");
			if (getAttributeValue(elem.children[i], "serverurl") == url) j = i;
        }
...
function getBaseContainer() {
    ...
    for (var i = 0; i < itemCount; i++) {        // find the media content video, photo or music
        if (getNMCPropertyText(list, "meta.pv:persistentID", i) == currentPersistentID)
            return getNMCPropertyText(list, "item.url", i);		// Twonky server has always this property
		var itemId = getNMCPropertyText(list.item[i], "meta.id");
        if (isCurrentPersistentID(itemId))
            return getNMCPropertyText(list, "item.url", i);		// other media server
    }
    if (itemCount == 1) {
		// check if it is a special item (no video, music or photo)
		if (itemHasProperty(child, "meta.pv:persistentID")) {
			navigateServerStatus = 2;			// base container not found -> no media
			return "";							// is music, photo or video
		}
		var itemId = getNMCPropertyText(list, "item.id", i);
        if (isCurrentPersistentID(itemId)) {
			navigateServerStatus = 2;			// base container not found -> no media
			return "";							// is music, photo or video
		}
...
function branchHtml(elem, index, personalRating) {
	...
	if (currentRenderer[currentPersistentID] == localdevice) 		// local device: show the play icon
		var beamButton = '<a id="' + idDiv + '" name="containerBeamButton" class="myLibraryBeamContainerNmcLocalDevice"' + htmltitle + onClickPlayContainer + '></a>';
	else															// renderer: show the beam icon
		var beamButton = '<a id="' + idDiv + '" name="containerBeamButton" class="myLibraryBeamContainerNmc"' + htmltitle + onClickPlayContainer + '></a>';
...
function leafHtmlVideo(elem, thumbnail, index) {
	...
	var htmlonclick = ' onclick="playBookmark(\'' + bookmark + '\', \'' + id + '\', \'' + url + '\', \'' + elemUrl + '\', \'' + escape(title) + '\', \'' + escape(artist) + '\', \'' + poster + '\')" ';
